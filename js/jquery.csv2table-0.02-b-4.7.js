...
(function($) {
	...
	$.csv2table={
		...
		sortDImg : (new Image()).src='data:image/gif;base64,R0lGODlhCwAJAIAAAIGBgf///yH5BAUUAAEALAAAAAALAAkAAAIOjI+pu+APEozqsUCvVgUAOw==',//'./img/icon-d.gif',   //Dafault sort IMG D
		sortAImg : (new Image()).src='data:image/gif;base64,R0lGODlhCwAJAIAAAICAgP///yH5BAUUAAEALAAAAAALAAkAAAINjI+pCmsAF5xpWoezKwA7' ,//'./img/icon-a.gif',   //Dafault sort IMG A
		setting  : [],
		data     : [],
		_rowsAry : [],
		_doc     : document,
	...
	$.fn.csv2table= function (url,setting){
		...
		$(contents).before('<div class="csv2table-loading">'+op.nowloadingMsg+' </div>' )
		...
		function  mkTable(id,rowsAry){
			...
			for (var k=0; k<col; k++) {
				...
				if(op.sortable)
				htm+= "<br><img id='"+id+"-sortimg-"+k+"' class='sortimg' src='"+sortimgsrc+"' border='0'>"
				htm+= "</th>";
