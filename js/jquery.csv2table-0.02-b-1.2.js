...
(function($) {
	...
	$.csv2table={
		...
		loadImg  : (new Image()).src='/~jimmy/yp/img/icon-loadinfo.gif',
		sortDImg : (new Image()).src='/~jimmy/yp/img/icon-d.gif',
		sortAImg : (new Image()).src='/~jimmy/yp/img/icon-a.gif',
		setting  : [],
		data     : [],
		_rowsAry : [],
		err      : []
	...
	$.fn.csv2table=function (url,setting){
		...
		$(contents).before('<div class="csv2table-loading"><img alt="loading" src="'+op.nowloadingImg+'"> '+op.nowloadingMsg+' </div>' );
		...
		function mkTable(id,data,sortType,colIndex){
			...
			for (k=0; k<col; k++) {
				...
				if(op.sortable){
					htm+="<br><a href='javascript:$.fn.csv2table.wrtTable(\"sortD\","+k+",\""+id+"\")'>"
					   + "<img alt='desc' src='"+$.csv2table.sortDImg+"' border='0'>"
					   + "</a><a href='javascript:$.fn.csv2table.wrtTable(\"sortA\","+k+",\""+id+"\")'>"
					   + "<img alt='asc' src='"+$.csv2table.sortAImg+"' border='0'></a>"
				}
				htm+= "</th>";
			}
			htm+= "</tr>";
			...
			var tableHtm=document.getElementById(id).innerHTML="<table>"+htm+"</table>";
		...
		function setCSS(id){
			$('#'+id+'').addClass(op.className_div)
			$('#'+id+' table').addClass(op.className_table)
			$('#'+id+' table th').addClass(op.className_table_th)
			$('#'+id+' table td').addClass(op.className_table_td)
		}
